...
class sendMail {
	...
	*******************************************/
	public function send () {
		...
		//处理From头,屏幕显示的发件人
		$head.="From: \"=?UTF-8?B?".base64_encode($this->fromName)."?=\" <{$this->fromMail}>"."\r\n";
		//处理To头,屏幕显示的收件人
		$head.="To: \"=?UTF-8?B?".base64_encode($this->toName)."?=\" <{$this->toMail}>"."\r\n";
		//处理MIME-Version头
		$head.="MIME-Version: 1.0\r\n";
		//处理分段标志
		$head.="Content-Type: multipart/mixed;boundary=\"{$boundary}\""."\r\n";
		//加入自己的头
		$head.="X-Sender-Server: {$this->serverName} \r\n";
		//换行一下你就知道
		$head.="\r\n";
		...
		$head_temp="--".$boundary."\r\n";		
		//加入邮件Mime类型和UTF8编码声明
		if($this->is_html) $head_temp.="Content-Type: text/html; charset=UTF-8"."\r\n";
		else $head_temp.="Content-Type: text/plain; charset=UTF-8"."\r\n";
		//加入内容是Base64编码的声明
